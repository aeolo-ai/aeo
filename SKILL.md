...
| `/aeo content redeploy <id> [--target shopify\|blog\|wordpress\|cafe24]` | Push the current article back to wherever it is deployed, in-place (keeps URL). Target is auto-detected from the deploy record; `--target` forces one. The hosted `blog` needs no push — it renders your article live — so it reports that instead. | [content-manage.md](references/content-manage.md) |
