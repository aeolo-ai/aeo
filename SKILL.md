...
| `strategy` | `show` · `update` · `visual update` | [strategy.md](references/strategy.md) |
...
| `content` | `idea <domain_id>`† · `list` · `feed` · `get <id>` · `import` · `generate` (explicit-only paid job) · `jobs` · `update <id>` · `preview <id>` · `deploy <id>` · `redeploy <id>` · `unpublish <id>` · `editions <id>` (locale fan-out) · `delete <id>` (deployed copies stay live — `unpublish` first) · `job cancel <jobId>` · `review <id>` | [content-manage.md](references/content-manage.md), [content-create.md](references/content-create.md), [content-idea.md](references/content-idea.md), [content-review.md](references/content-review.md) |
...
| `prompts` | `portfolio <domain_id>`† (the **only** safe way to restructure a live tracked set) · `audit <domain_id>`† · `list` · `add` · `update <id>` (`--regions US,GB` = standing target markets, measured once per market per check) · `delete <id>` · `generate` (seeds an **empty** set only — can shrink a live portfolio; use `portfolio` instead) | [brand.md](references/brand.md), [prompt-portfolio.md](references/prompt-portfolio.md), [prompt-audit.md](references/prompt-audit.md) |
