...
| `site template` | `show` (where the article slot sits — warns when it landed in the header/footer) · `read` · `edit` (exact-match patches; chat/MCP via `templateUpdate`, confirm first) · `revisions` · `rollback <n>` · `recapture` | [channels.md](references/channels.md) |
