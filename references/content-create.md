...
| `content` | string (min 100, markdown) | Yes | — |
| `articleType` | enum | — | `blog` |
| `targetKeywords` | string[] (1–20) | Yes | — |
| `language` | enum | — | `en` |
| `rationale` | string (max 1000) | Yes | — |
...
During the import flow, if a `<!-- schema: ... -->` comment exists at the top of the article, parse it and map to the `schemaTypes` field. Otherwise, use automatic mapping based on articleType.
